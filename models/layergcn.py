...
class LayerGCN(GeneralRecommender):
    ...
    def forward(self):
        ego_embeddings = self.get_ego_embeddings()
        all_embeddings = ego_embeddings
        # FIX: include ego (layer-0) in the list.
        # Without this, if cosine_similarity ≈ 0 at random init,
        # all propagated embeddings collapse to ~0 → scores ≈ 0 →
        # BPR loss stuck at 0.693*N → vanishing gradient → model never learns.
        ...
        # Use mean (as in original paper) instead of sum
        ui_all_embeddings = torch.mean(torch.stack(embeddings_layers, dim=0), dim=0)
        user_all_embeddings, item_all_embeddings = torch.split(ui_all_embeddings, [self.n_users, self.n_items])
        return user_all_embeddings, item_all_embeddings
