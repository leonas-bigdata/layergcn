...
class LayerGCN(GeneralRecommender):
    ...
    def pre_epoch_processing(self):
        ...
        if self.edge_values.device != self.device:
            self.edge_values = self.edge_values.to(self.device)
        if self.edge_indices.device != self.device:
        ...
        if self.pruning_random:
            # Random pruning
            perm = torch.randperm(self.edge_values.size(0), device=self.device)
            keep_idx = perm[:keep_len]
        else:
            # Prune by probability (theo paper gốc)
            keep_idx = torch.multinomial(self.edge_values, keep_len)

        self.pruning_random = not self.pruning_random   # toggle

        # Lấy indices và values
        keep_indices = self.edge_indices[:, keep_idx]
        keep_values = self._normalize_adj_m(keep_indices, torch.Size((self.n_users, self.n_items)))

        # Undirected graph
        all_values = torch.cat((keep_values, keep_values))
        keep_indices = keep_indices.clone()   # tránh modify original
        keep_indices[1] += self.n_users
    ...
    def forward(self):
        ...
        embeddings_layers = [ego_embeddings]
